...
In the "order-cancellation-preds.ipynb", I start off by creating invoice-level variables, and make predictions with those to set the baseline. After that, I add customer-level variables to show how much of an improvement there can be, which is quite considerable. I make use of expanding-window CV, proper for time series data, and StackingClassifier (includes 5 models) to make final  predictions on the unseen data. If one wants to take this any further, it's possible to:
...
* ~Instead of using a two-stage tuning, where each base estimators hyperparameters tuned independent of StackingClassifier, search for the best combination of base estimators' hyper-parameters and the StackingClassifier together.~
